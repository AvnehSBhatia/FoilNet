...
class GeneticCLOptimizer:
    ...
    def __init__(self, 
        ...
        self.stage1_best_peak_cl = 0.0  # Stage 1 now optimizes for peak CL
        self.stage2_best_peak_ld = 0.0  # Stage 2 now optimizes for peak L/D
        self.stage3_best_avg_ld = 0.0  # Stage 3 now optimizes for average L/D
        ...
        self.avg_ld_preservation_threshold = 0.98  # Must maintain 98% of stage 3 avg L/D (2% max drop)
    ...
    def make_individual(self) -> dict:
        ...
        # Set xy_coords_display if it wasn't set in the clipping branch
        if 'xy_coords_display' not in locals():
            xy_coords_display = None
    ...
    def _stage_fitness(self, aero_data: dict) -> float:
        if not self.stage1_complete:
            # Stage 1: Optimize for peak CL at peak L/D (flipped from stage 2)
            # Optimize for peak CL without constraints
            return aero_data['CL'] * 100
        elif not self.stage2_complete:
            # Stage 2: Optimize for peak L/D only (flipped from stage 1)
            # Check if peak CL is preserved within 2% threshold
            current_peak_cl = aero_data['CL']
            
            if current_peak_cl >= self.stage1_best_peak_cl * self.peak_cl_preservation_threshold:
                # Return peak L/D
                return aero_data['L/D']
            else:
                # Strong penalty if peak CL drops below 2% threshold
                penalty_factor = 0.01  # Very strong penalty
                return current_peak_cl * penalty_factor
        elif not self.stage3_complete:
            # Stage 3: Optimize for average L/D over ±4° range around peak L/D (flipped from stage 4)
            current_peak_ld = aero_data['L/D']
            current_peak_cl = aero_data['CL']
            
            # Check if both peak CL and peak L/D are preserved within 2% threshold
            cl_preserved = current_peak_cl >= self.stage1_best_peak_cl * self.peak_cl_preservation_threshold
            ld_preserved = current_peak_ld >= self.stage2_best_peak_ld * self.peak_ld_preservation_threshold
            
            if cl_preserved and ld_preserved:
                ...
                # Strong penalty if either peak CL or peak L/D drops below 2% threshold
                penalty_factor = 0.01  # Very strong penalty
                return min(current_peak_cl, current_peak_ld) * penalty_factor
        else:
            # Stage 4: Optimize for average CL over ±4° range around peak L/D (flipped from stage 3)
            current_peak_ld = aero_data['L/D']
            current_peak_cl = aero_data['CL']
    ...
    def run(self, initial_naca: str = None):
        ...
        # Stage 1: Optimize for peak CL at peak L/D (flipped from stage 2)
        print(f"\n" + "="*60)
        print(f"STAGE 1: Optimizing for Peak CL at Peak L/D")
        print(f"="*60)
        print(f"Focus: Maximize peak CL at peak L/D angle")
        print(f"No preservation constraints in this stage")
        ...
        for generation in range(self.generations):
            ...
            # Get actual CL value for display (fitness is CL * 100)
            actual_cl = best_individual['aerodynamic_data']['CL'] if best_individual.get('aerodynamic_data') else best_fitness / 100
            print(f"Best CL: {actual_cl:.3f} (fitness: {best_fitness:.3f})")
            ...
            if generations_without_improvement >= self.early_stopping_patience:
                ...
                print(f"Best CL achieved: {actual_cl:.3f}")
                break
        ...
        # Stage 1 complete - record best peak CL
        self.stage1_complete = True
        # Extract actual CL value from best individual
        best_cl_value = best_individual['aerodynamic_data']['CL'] if best_individual.get('aerodynamic_data') else best_fitness_ever / 100
        self.stage1_best_peak_cl = best_cl_value
        ...
        print(f"\n" + "="*60)
        print(f"STAGE 1 COMPLETE: Peak CL = {self.stage1_best_peak_cl:.3f}")
        print(f"Switching to Stage 2: Optimizing for peak L/D")
        print(f"Must maintain peak CL ≥ {self.stage1_best_peak_cl * self.peak_cl_preservation_threshold:.3f} ({self.peak_cl_preservation_threshold*100:.0f}% of best)")
        print(f"Stage 2 focus: Maximize peak L/D while preserving peak CL within 2%")
        print(f"="*60)
        
        # Stage 2: Optimize for peak L/D while preserving peak CL (flipped from stage 1)
        print(f"\nStage 2: Optimizing for peak L/D (preserving peak CL)")
        print(f"Additional generations: {self.stage2_generations}")
        print(f"Peak L/D optimization: Maximize L/D ratio")
        print(f"Stage 2 early stopping patience: 50 generations")
        
        # Reset early stopping for stage 2 with 50 generation patience
        generations_without_improvement = 0
        stage2_best_fitness = 0.0
        ...
        # Stage 2 complete - record best peak L/D
        self.stage2_complete = True
        self.stage2_best_peak_ld = stage2_best_fitness
        ...
        print(f"\n" + "="*60)
        print(f"STAGE 2 COMPLETE: Peak L/D = {self.stage2_best_peak_ld:.3f}")
        print(f"Switching to Stage 3: Optimizing for average L/D over ±4° range around peak L/D")
        print(f"Must maintain peak CL ≥ {self.stage1_best_peak_cl * self.peak_cl_preservation_threshold:.3f} ({self.peak_cl_preservation_threshold*100:.0f}% of best)")
        print(f"Must maintain peak L/D ≥ {self.stage2_best_peak_ld * self.peak_ld_preservation_threshold:.3f} ({self.peak_ld_preservation_threshold*100:.0f}% of best)")
        print(f"Stage 3 focus: Maximize average L/D over ±4° range around peak L/D")
        print(f"="*60)
        
        # Stage 3: Optimize for average L/D over ±4° range around peak L/D (flipped from stage 4)
        print(f"\nStage 3: Optimizing for average L/D over ±4° range (preserving peaks)")
        ...
        # Stage 3 complete - record best average L/D
        self.stage3_complete = True
        self.stage3_best_avg_ld = stage3_best_fitness
        ...
        print(f"\n" + "="*60)
        print(f"STAGE 3 COMPLETE: Average L/D = {self.stage3_best_avg_ld:.3f}")
        print(f"Switching to Stage 4: Optimizing for average CL over ±4° range around peak L/D")
        print(f"Must maintain peak CL ≥ {self.stage1_best_peak_cl * self.peak_cl_preservation_threshold:.3f} ({self.peak_cl_preservation_threshold*100:.0f}% of best)")
        print(f"Must maintain peak L/D ≥ {self.stage2_best_peak_ld * self.peak_ld_preservation_threshold:.3f} ({self.peak_ld_preservation_threshold*100:.0f}% of best)")
        print(f"Stage 4 focus: Maximize average CL over ±4° range around peak L/D")
        print(f"="*60)
        
        # Stage 4: Optimize for average CL over ±4° range around peak L/D (flipped from stage 3)
        print(f"\nStage 4: Optimizing for average CL over ±4° range (preserving peaks)")
        ...
        print(f"Stage 1: Peak CL = {self.stage1_best_peak_cl:.3f}")
        print(f"Stage 2: Peak L/D = {self.stage2_best_peak_ld:.3f}")
        print(f"Stage 3: Average L/D = {self.stage3_best_avg_ld:.3f}")
        ...
        # Check if peak CL and peak L/D were preserved
        final_peak_cl = final_best['aerodynamic_data']['CL']
        final_peak_ld = final_best['aerodynamic_data']['L/D']
        if final_peak_cl >= self.stage1_best_peak_cl * self.peak_cl_preservation_threshold:
            print(f"✓ Peak CL preserved: {final_peak_cl:.3f} ≥ {self.stage1_best_peak_cl * self.peak_cl_preservation_threshold:.3f}")
        else:
            print(f"⚠ Peak CL not preserved: {final_peak_cl:.3f} < {self.stage1_best_peak_cl * self.peak_cl_preservation_threshold:.3f}")
        if final_peak_ld >= self.stage2_best_peak_ld * self.peak_ld_preservation_threshold:
            print(f"✓ Peak L/D preserved: {final_peak_ld:.3f} ≥ {self.stage2_best_peak_ld * self.peak_ld_preservation_threshold:.3f}")
        else:
            print(f"⚠ Peak L/D not preserved: {final_peak_ld:.3f} < {self.stage2_best_peak_ld * self.peak_ld_preservation_threshold:.3f}")
    ...
    def early_stop_stats(self):
        ...
        return {
            ...
            'stage1_best_peak_cl': self.stage1_best_peak_cl,
            'stage2_best_peak_ld': self.stage2_best_peak_ld,
            ...
            'peak_cl_preserved': (self.stage1_best_peak_cl > 0 and 
                                 stage2_best >= self.stage1_best_peak_cl * self.peak_cl_preservation_threshold),
...
GeneticCLOptimizer = GeneticCLOptimizer
